...
default_nq_mode="${NQ_RUN_NQ:-y}"    # y/l/n
default_bt_mode="${NQ_RUN_BT:-n}"    # y/n
ORIG_ARGS=("$@")
...
Environment:
  NQ_HQ_MODE=f|y|v|n   (default f, fast hardware test)
  NQ_RUN_IQ=y|n        (default y)
  NQ_RUN_NQ=y|l|n      (default y)
...
function post_cleanup(){
    chroot_run umount -R /dev &> /dev/null || true
    clear_mount
    ...
    # keep local result by default; only remove temp work dir when user disables local keep
    if [[ "$keep_local_result" -ne 1 ]]; then
        if [[ "$work_dir" == *"nodequality"* ]]; then
            rm -rf "${work_dir}"/
        else
            echo "$(L err01)"
...
function post_check_mount(){
    if mount | grep -q "nodequality$current_time" ; then
...
function ask_question(){
    ...
    if [[ "$non_interactive" -eq 1 ]]; then
        run_hardware_quality_test="${default_hq_mode}"
        run_ip_quality_test="${default_iq_mode}"
        run_net_quality_test="${default_nq_mode}"
        run_net_trace_test="${default_bt_mode}"
        return
    fi

    echo -en "${yellow}$(L ask_hq)${reset}"
    read run_hardware_quality_test
    run_hardware_quality_test=${run_hardware_quality_test:-$default_hq_mode}

    echo -en "${yellow}$(L ask_iq)${reset}"
    read run_ip_quality_test
    run_ip_quality_test=${run_ip_quality_test:-$default_iq_mode}

    echo -en "${yellow}$(L ask_nq)${reset}"
    read run_net_quality_test
    run_net_quality_test=${run_net_quality_test:-$default_nq_mode}

    echo -en "${yellow}$(L ask_bt)${reset}"
    read run_net_trace_test
    run_net_trace_test=${run_net_trace_test:-$default_bt_mode}
}
...
function main(){
    ...
    if [[ "$run_in_background" -eq 1 && -z "${NQ_BG_CHILD:-}" ]]; then
        local log_root="${HOME}/.nodequality-logs"
        mkdir -p "$log_root"
        bg_log_file="$log_root/nodequality_${current_time}.log"
        nohup env NQ_BG_CHILD=1 NQ_HQ_MODE="$default_hq_mode" NQ_RUN_IQ="$default_iq_mode" NQ_RUN_NQ="$default_nq_mode" NQ_RUN_BT="$default_bt_mode" \
            bash "$0" "${ORIG_ARGS[@]/-B}" > "$bg_log_file" 2>&1 < /dev/null &
